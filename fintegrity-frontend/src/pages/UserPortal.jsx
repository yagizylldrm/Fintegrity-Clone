...
export default function UserPortal({ username, onLogout, auth, setAuth }) {
  ...
  const toggleTheme = async () => {
    ...
    try {
      await axios.put("http://localhost:8000/api/auth/theme", {
    ...
    <div className={`theme-${auth?.theme || 'current'} min-h-screen font-sans flex flex-col ${
      ...
      <header className="glass-panel sticky top-0 z-10 px-8 py-4 flex items-center justify-between border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight text-white">Bireysel Portal</h1>
            <p className="text-xs text-slate-400 font-medium">Fintegrity Güvenli İşlem Ağı</p>
          </div>
        </div>
      ...
      <main className="flex-1 p-8 max-w-5xl mx-auto w-full grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 space-y-4">
          ...
          >
            <ShieldCheck size={20} /> Akıllı Sözleşmeler
          </button>
          <button 
          ...
          <div className="mt-8 p-6 glass-panel rounded-2xl border border-slate-700/50 bg-slate-800/20">
            <h3 className="text-sm font-bold text-slate-300 mb-2">Cüzdan Bakiyesi</h3>
        ...
        <div className="md:col-span-2">
          {activeTab === 'payment' && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="glass-panel p-8 rounded-3xl border border-slate-700/50">
              <div className="flex items-center gap-3 mb-8 pb-6 border-b border-slate-800">
                ...
                <div>
                  <h2 className="text-2xl font-black text-white">Para Transferi</h2>
                  <p className="text-sm text-slate-400">Blockchain tabanlı güvenli ödeme ağı</p>
                </div>
              </div>
              ...
              {success ? (
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="py-12 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-slate-400 max-w-sm mb-8">Transferiniz Fintegrity yapay zeka denetiminden başarıyla geçti ve blockchain ağına kaydedildi.</p>
                ...
                <form onSubmit={handlePayment} className="space-y-6">
                  <div>
                    <label className="text-xs text-slate-400 uppercase font-bold mb-2 block">Alıcı Cüzdan / IBAN</label>
                    <input 
                      type="text" 
                      value={recipient}
                  ...
                  <div>
                    <label className="text-xs text-slate-400 uppercase font-bold mb-2 block">Gönderilecek Tutar (₺)</label>
                    <input 
                      type="number" 
                      value={amount}
                  ...
                  <div className="pt-4">
                    ...
                    >
                      {isProcessing ? <><Activity className="animate-spin" /> Güvenlik Taraması Yapılıyor...</> : 'Transferi Başlat'}
                    </button>
                  </div>
                  <p className="text-center text-xs text-slate-500 flex items-center justify-center gap-1 mt-4">
                    <ShieldCheck size={14} /> Yapay Zeka destekli dolandırıcılık koruması aktiftir.
                  </p>
                </form>
              )}
            </motion.div>
          ...
          {activeTab === 'history' && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="glass-panel p-8 rounded-3xl border border-slate-700/50">
              <h2 className="text-2xl font-black text-white mb-6 border-b border-slate-800 pb-4">Son İşlemler</h2>
              <div className="space-y-4">
                {[
                  { id: '1', to: 'TR12 3456...', amount: 1500, date: 'Bugün, 14:30', status: 'Onaylandı' },
                  { id: '2', to: 'TR98 7654...', amount: 450, date: 'Dün, 09:15', status: 'Onaylandı' },
                  { id: '3', to: 'E-Ticaret A.Ş.', amount: 12500, date: '12 Haziran', status: 'Onaylandı' },
                ].map(tx => (
