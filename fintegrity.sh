...
show_help() {
    echo "Fintegrity MVP Yönetim Aracı"
...
check_dependencies() {
    echo "Bağımlılık durumu kontrol ediliyor..."
    NEED_INSTALL=0

    # 1. Backend venv kontrolü
    if [ ! -d "$DIR/backend/venv" ]; then
        echo "Uyarı: Backend sanal ortamı (venv) bulunamadı."
        NEED_INSTALL=1
    fi

    # 2. Blockchain node_modules kontrolü
    if [ ! -d "$DIR/blockchain/node_modules" ]; then
        echo "Uyarı: Blockchain kütüphaneleri (node_modules) bulunamadı."
        NEED_INSTALL=1
    fi

    # 3. Frontend node_modules kontrolü
    if [ ! -d "$DIR/fintegrity-frontend/node_modules" ]; then
        echo "Uyarı: Frontend kütüphaneleri (node_modules) bulunamadı."
        NEED_INSTALL=1
    fi

    # Eğer eksik varsa make install komutunu tetikle
    if [ $NEED_INSTALL -eq 1 ]; then
        echo "Eksik bağımlılıklar tespit edildi. 'make install' otomatik olarak başlatılıyor..."
        cd "$DIR"
        if command -v make >/dev/null 2>&1; then
            make install
            if [ $? -ne 0 ]; then
                echo "Hata: Bağımlılıkların otomatik kurulumu (make install) başarısız oldu!"
                exit 1
            fi
        else
            echo "Hata: Sistemde 'make' aracı bulunamadı, otomatik kurulum gerçekleştirilemiyor."
            exit 1
        fi
        echo "Tüm bağımlılıklar başarıyla kuruldu. Servisler başlatılıyor..."
        echo "---------------------------------------------"
    else
        echo "Tüm bağımlılıklar mevcut. Kuruluma gerek yok."
        echo "---------------------------------------------"
    fi
}

start_services() {
    # Servisleri ayağa kaldırmadan önce bağımlılıkları denetle
    check_dependencies

    echo "Fintegrity platformu başlatılıyor..."
    echo "---------------------------------------------"

    # 1. Start Blockchain (Hardhat Node)
    if [ -f "$PID_DIR/blockchain.pid" ] && kill -0 $(cat "$PID_DIR/blockchain.pid") 2>/dev/null; then
        echo "Uyarı: Blockchain ağı zaten çalışıyor (PID: $(cat "$PID_DIR/blockchain.pid"))."
    else
    ...
    if [ $DEPLOY_STATUS -ne 0 ]; then
        echo "Hata: Akıllı sözleşme dağıtımı başarısız oldu! Detaylar için 'logs/deploy.log' dosyasını inceleyin."
        stop_services
        exit 1
    fi
    ...
    if [ -f "$PID_DIR/backend.pid" ] && kill -0 $(cat "$PID_DIR/backend.pid") 2>/dev/null; then
        echo "Uyarı: Backend sunucusu zaten çalışıyor (PID: $(cat "$PID_DIR/backend.pid"))."
    else
    ...
    if [ -f "$PID_DIR/frontend.pid" ] && kill -0 $(cat "$PID_DIR/frontend.pid") 2>/dev/null; then
        echo "Uyarı: Frontend geliştirme sunucusu zaten çalışıyor (PID: $(cat "$PID_DIR/frontend.pid"))."
    else
...
stop_services() {
    echo "Fintegrity platformu durduruluyor..."
    echo "---------------------------------------------"
...
status_services() {
    echo "Fintegrity Servis Durum Raporu"
    echo "---------------------------------------------"

    # Check Blockchain
    if [ -f "$PID_DIR/blockchain.pid" ] && kill -0 $(cat "$PID_DIR/blockchain.pid") 2>/dev/null; then
        echo "Blockchain (Hardhat): ÇALIŞIYOR (PID: $(cat "$PID_DIR/blockchain.pid"), Port: 8545)"
    else
        echo "Blockchain (Hardhat): DURDURULDU"
    fi

    # Check Backend
    if [ -f "$PID_DIR/backend.pid" ] && kill -0 $(cat "$PID_DIR/backend.pid") 2>/dev/null; then
        echo "Backend (FastAPI)  : ÇALIŞIYOR (PID: $(cat "$PID_DIR/backend.pid"), Port: 8000)"
    else
        echo "Backend (FastAPI)  : DURDURULDU"
    fi
...
show_logs() {
    echo "Canlı Log Takibi Başlatıldı (Çıkmak için CTRL+C)"
    echo "---------------------------------------------"
